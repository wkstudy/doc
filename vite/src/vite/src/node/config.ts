...
export interface InlineConfig extends UserConfig {
  configFile?: string | false; // WK 命令行中指定配置文件 如 vite --config my-config.js
  envFile?: false; // WK 指定.env文件
}
...
export async function loadConfigFromFile(
  ...
  let resolvedPath: string | undefined; // WK 配置文件路径
  let isTS = false;
  let isMjs = false; // WK 是否是es6
  let dependencies: string[] = [];

  // check package.json for type: "module" and set `isMjs` to true
  // WK package.json的type属性参考https://www.cnblogs.com/zmztya/p/14419578.html
  try {
  ...
  if (configFile) {
    ...
    // otherwise from cwd
    // WK 没有在命令行指定的话按照.js .mjs .ts的顺序在根目录里找vite.config
  ...
  // WK 找到配置文件并取出值放到 userConfig
  try {
    let userConfig: UserConfigExport | undefined;

    if (isMjs) {
      const fileUrl = require("url").pathToFileURL(resolvedPath); // WK pathToFileUrl返回文件 URL 对象
      if (isTS) {
        // before we can register loaders without requiring users to run node
        // with --experimental-loader themselves, we have to do a hack here:
        // bundle the config file w/ ts transforms first, write it to disk,
        // load it with native Node ESM, then delete the file.
        // WK ts文件没法直接使用，得先转换成js
    ...
    return {
      ...
      config,
      dependencies, // WK-TODO 不清楚这个dependencies到底是什么
    };
  } catch (e) {
