...
export default class Bundle {
  constructor(options) {
    ...
    // wk resolveId就是将一个文件转化为一个id（一般就是该文件的路径）
  ...
  fetchModule(id, importer) {
    ...
    return this.load(id)
      ...
      .then((source) => {
        ...
        throw new Error(
          `Error loading ${id}: load hook should return a string, a { code, map } object, or nothing/null`
        );
      })
      .then((source) => {
      ...
      .then((source) => {
        ...
        // wk 将该module依赖的模块都全部处理（resolveid => load=>  fetchmodule）
    ...
    return mapSequence(module.sources, (source) => {
      ...
      return (resolvedId ? Promise.resolve(resolvedId) : this.resolveId(source, module.id)).then((resolvedId) => {
        ...
        if (isExternal) {
          ...
          // wk 如果是从其他地方引入的，那么就开始fetchModule
  ...
  render(options = {}) {
    ...
    code = transformBundle(code, this.plugins, bundleSourcemapChain).replace(
